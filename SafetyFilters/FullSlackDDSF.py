...
class FullSlackDDSafetyFilter(DDSafetyFilter):
    ...
    def __init__(self, sys: LTI, io_data: IOData, full_slack_sf_params: FullSlackSFParams) -> None:
        L, lag, R = full_slack_sf_params.L, full_slack_sf_params.lag, full_slack_sf_params.R
        ...
        self._lag = lag
        self._R = R
        self._lam_alph = full_slack_sf_params.lam_alph
        self._lam_sig = full_slack_sf_params.lam_sig
        assert full_slack_sf_params.sf_params.steps == self._n, f"Given steps {full_slack_sf_params.sf_params.steps}, not compatible with this SF!"
        DDSafetyFilter.__init__(self, full_slack_sf_params.sf_params)

        self._io_data.update_depth(self._L+self._lag)

        self._epsilon = full_slack_sf_params.epsilon
