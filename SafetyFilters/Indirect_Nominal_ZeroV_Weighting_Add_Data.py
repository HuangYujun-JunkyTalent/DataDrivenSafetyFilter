...
@dataclass
class IndirectNominalZeroVWeightingAddDataParams:
    L: int
...
class IndirectNominalZeroVWeightingAddDataFilter(DDSafetyFilter):
    ...
    def __init__(self, 
                 sys: LinearizedErrorKinematicAcceModel, io_data: IOData, params: IndirectNominalZeroVWeightingAddDataParams,
                ) -> None:
        ...
        constraints.append(self._u[-self._m*l_terminal:] == np.hstack((self._u_s,) * l_terminal))
        constraints.append(self._y[-self._p*l_terminal:] == np.hstack((self._y_s,) * l_terminal)) # terminal constraints
