...
class NominalFewSlackDDSafetyFilter(DDSafetyFilter):
    ...
    def __init__(self, sys: LTI, io_data: IOData, few_slack_sf_params: NominalFewSlackSFParams) -> None:
        ...
        for k in range(self._L-self._n):
            ...
            constraints.append(cp.norm_inf(self._y[i*self._p:(i+1)*self._p]) + 1 <= sys.b_u[0,0])
