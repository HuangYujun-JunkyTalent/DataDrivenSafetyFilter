...
class NominalFewSlackTightenedDDSafetyFilter(DDSafetyFilter):
    ...
    def __init__(self, sys: LTI, io_data: IOData, few_slack_tightened_sf_params: NominalFewSlackTightenedSFParams) -> None:
        ...
        constraints = [A[:,self._lag*self._m:-self._p*self._lag] @ self._u == \
                       self._y - self._sigma - A[:,:self._lag*self._m]@self._xi_t[:self._lag*self._m] - A[:,-self._p*self._lag:]@self._xi_t[-self._p*self._lag:]]
