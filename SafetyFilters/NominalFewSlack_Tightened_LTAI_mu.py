...
class NominalFewSlackTightenedDDSafetyFilter(DDSafetyFilter):
    ...
    def __init__(self, sys: Union[LTI,LATI], io_data: IOData, few_slack_tightened_sf_params: NominalFewSlackTightenedSFParams) -> None:
        ...
        if few_slack_tightened_sf_params.sf_params.steps != self._n:
            warn(f"Given steps {few_slack_tightened_sf_params.sf_params.steps}!")
        ...
        l_terminal = self._n
        # constraints.append(self._u[-self._m*l_terminal::self._m] == np.zeros((l_terminal,)))
        constraints.append(self._u[-self._m*l_terminal:] == np.zeros((self._m*l_terminal,))) # u
        # constraints.append(self._y[-self._p*self._n:] == np.zeros((self._p*l_terminal,)))
        # constraints.append(self._y[-self._p*l_terminal::self._p] == np.zeros((l_terminal)))
        constraints.append(self._y[-self._p*l_terminal+2::self._p] == -sys._v_0*np.ones((l_terminal))) # v
        for i in range(1,l_terminal):
            ...
            # constraints.append(self._u[-self._m*i+1] == self._u[-self._m*(i+1)+1])
        # constraints.append(self._y[-self._p*l_terminal::self._p] == np.zeros((l_terminal)))
        # constraints.append(self._y[-self._p*l_terminal+1::self._p] == np.zeros((l_terminal)))
        # terminal constraints

        l_constrained = self._L
        # l_constrained = self._L-l_terminal
