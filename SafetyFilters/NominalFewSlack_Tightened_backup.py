...
class NominalFewSlackTightenedDDSafetyFilter(DDSafetyFilter):
    ...
    _R: np.matrix
    _c_1: float # tightening constant
    ...
    def __init__(self, sys: LTI, io_data: IOData, few_slack_tightened_sf_params: NominalFewSlackTightenedSFParams) -> None:
        ...
        self._c_1 = few_slack_tightened_sf_params.c_1
        ...
        constraints = [self._io_data.H_output_noised_part((self._lag, self._lag+self._L)) @ self._alpha == self._y[self._lag*self._p:] - self._sigma]
        ...
        if sys.A_y is not None:
            ...
            for j in range(J):
                Delta_j = pt.Polytope(lb=-j*self._c_1*np.ones(sys.p), ub=j*self._c_1*np.ones(sys.p))
                P_j = P_y-Delta_j
                P_j.minimize_H_rep()
                P_y_list.append(P_j)
