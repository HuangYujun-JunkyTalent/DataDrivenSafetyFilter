...
class SafetyFilterForTrackAddData:
    '''
    ...
    _safety_filters: List # list of safety filters
    ...
    def __init__(self, 
        ...
        self._previous_traj = []
        self.first_filtering = True # filtering the first point will always need a new dataset
    ...
    def filter(self, u_obj: np.matrix) -> Tuple[np.matrix, str, float]:
        ...
        u_list = []
        error_state_list = []
        for i in range(safety_filter._lag, 0, -1):
            u_list.append(self._previous_traj[-i][0])
            ...
            error_state_list.append(error_state_no_v0)
        self._previous_traj = [] # clear the global trajectory
        return safety_filter.filter(u_list, error_state_list, u_obj, segment_changed)
