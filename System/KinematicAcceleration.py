...
@dataclass
class KinematicAccelerationModelParams:
    l_r: float
    l_f: float
    Ts: float

class KinematicAccelerationModel:
    '''
    Input [a, beta] as np.ndarray; output [x_p_t, y_p_t, Psi_t, v_t] as np.ndarray
    '''
    _l_r: float
    ...
    _u: cas.SX # input varialbe, [a, beta]
    _f_c: cas.SX # kinematic ode, input is [a, beta]
    _F_c: cas.Function # integrator of kinematic system

    def __init__(self, params: KinematicAccelerationModelParams, state0: np.ndarray) -> None:
        ...
        self._f_c[0] = self._z[3]*cas.cos(self._z[2]+self._u[1])
        self._f_c[1] = self._z[3]*cas.sin(self._z[2]+self._u[1])
        self._f_c[2] = self._z[3]*cas.sin(self._u[1])/self._l_r
...
@dataclass
class LinearizedKinematicAccelerationModelParams:
    kinematic_acceleration_params: KinematicAccelerationModelParams
    # state0: np.ndarray # initial system state [x_p_0, y_p_0, Psi_0, v_0]

    state0s: np.ndarray # initial condition for steady trajectory
    input_s: np.ndarray # control input for steady trajectory, [a_0, beta_0], a_0 must be 0!
...
class LinearizedKinematicAccelerationModel(LTI):
    '''
    Kinematic Model of Chronos, includes a Linearization around a steady trajectory
    '''
    kinematic_model: KinematicAccelerationModel
    ...
    def __init__(self, params: LinearizedKinematicAccelerationModelParams, state0: np.ndarray) -> None:
        
        self.kinematic_model = KinematicAccelerationModel(params.kinematic_acceleration_params, state0)
        assert params.input_s[0] == 0, f"Given acceleration {params.input_s[0]}, not zero!"
        self._ts = params.ts
        ...
        self._z0s = cas.SX.sym('_z0s', 4)
        self._zs = cas.SX.sym('_zs', 4)
        # Order of assigning variables matters! it will affect final expressions
        self._zs[3] = self._z0s[3]
        self._zs[2] = self._z0s[2] + self._zs[3] * cas.sin(self._us[1]) / self.kinematic_model._l_r * self._t
    ...
    def get_lti(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        ...
        A_c_LTI = np.array(self._A_tilde_fun(self._ts, self._input_s, self._state0s))
        B_c_LTI = np.array(self._B_tilde_fun(self._ts, self._input_s, self._state0s))
        # ues exact discretization
        o = 2 # number of variables observable
        A, B, C, D , _ = signal.cont2discrete((A_c_LTI, B_c_LTI, np.hstack(( np.eye(o),np.zeros((o,4-o)) )), np.zeros((o,2))), self.kinematic_model.Ts)
    ...
    def get_zs(self) -> np.ndarray:
        ...
        if self._input_s[1] == 0: # beta_s equals 0
            x_s = self._state0s[0] + self._ts*cas.cos(self._state0s[2])*self._state0s[3]
            y_s = self._state0s[1] + self._ts*cas.sin(self._state0s[2])*self._state0s[3]
            Phi_s = self._state0s[2]
            v_s = self._state0s[3]
            return np.array([x_s, y_s, Phi_s, v_s])
        else:
