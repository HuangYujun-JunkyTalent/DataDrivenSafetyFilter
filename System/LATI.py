...
class LATIParams:
    ...
    C: np.matrix
    D: np.matrix
    c: np.matrix # affine term for state
    r: np.matrix # affine term for output
    # # Initial state
    # x0: np.matrix
    # Constraints
...
class LATI:
    ...
    def __init__(self, lati_params: LATIParams, x0: np.matrix):
        # Controllability and Minimal Represention are not checked!
        A, B, C, D, c, r = lati_params.A, lati_params.B, lati_params.C, lati_params.D, lati_params.c, lati_params.r
        ...
        self._A, self._B, self._C, self._D, self._c, self._r, self._x = A, B, C, D, c, r, x0

        if lati_params.A_y is not None:
            assert lati_params.A_y.shape[1] == self._p and lati_params.b_y.shape == (lati_params.A_y.shape[0], 1), f"Wrong output constraint shape, shape of A_y: {lati_params.A_y.shape}, shape of b_y: {lati_params.b_y.shape}, system p: {self._p}"
        if lati_params.A_u is not None:
            assert lati_params.A_u.shape[1] == self._m and lati_params.b_u.shape == (lati_params.A_u.shape[0], 1), f"Wrong input constraint shape, shape of A_u: {lati_params.A_u.shape}, shape of b_u: {lati_params.b_u.shape}, system m: {self._m}"

        self._A_y, self._A_u, self._b_y, self._b_u = lati_params.A_y, lati_params.A_u, lati_params.b_y, lati_params.b_u

        if lati_params.A_n is not None:
            assert lati_params.A_n.shape[1] == self._p and lati_params.b_n.shape == (lati_params.A_n.shape[0], 1), f"Wrong noise constraint shape, shape of A_n: {lati_params.A_n.shape}, shape of b_n: {lati_params.b_u.shape}, system p: {self._p}"
        self._A_n, self._b_n = lati_params.A_n, lati_params.b_n
    ...
    def step(self, u: np.matrix) -> Tuple[np.matrix, np.matrix]:
        ...
        y: np.matrix = np.matmul(self._C, self._x) + np.matmul(self._D, u) + self._r
        self._x = np.matmul(self._A, self._x) + np.matmul(self._B, u) + self._c
