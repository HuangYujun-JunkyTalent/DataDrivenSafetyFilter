...
class trackGenerator:
    ...
    def plotPoints(self, ax):
        ax.plot(self.xCoords,self.yCoords, label='Track Center', color='black', linestyle='--', linewidth=1)
        mean_x = (np.max(self.xCoords)+np.min(self.xCoords))/2.0
        mean_y = (np.max(self.yCoords)+np.min(self.yCoords))/2.0
        # ax.plot(mean_x,mean_y,'go',label='Center',markersize=5)
        ax.plot(self.xCoords + self.yRate*self.track_width/2,self.yCoords-self.xRate*self.track_width/2, color='black', linestyle='-', linewidth=1)
        ax.plot(self.xCoords - self.yRate*self.track_width/2,self.yCoords+self.xRate*self.track_width/2, color='black', linestyle='-', linewidth=1)
        ax.legend()
